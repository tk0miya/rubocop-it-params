...
To release a new version, update the version number in `lib/rubocop/prefer_it_parameter/version.rb`, add the matching section to `CHANGELOG.md`, and merge it into `main`. The [Release workflow](.github/workflows/release.yml) then tags the version and publishes the gem to [rubygems.org](https://rubygems.org). Publishing is skipped when the version is already on RubyGems, so the workflow can be re-run (`workflow_dispatch`) after a failure.
